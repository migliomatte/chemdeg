...
2. if the 95% confidence interval of $n$ does not include integer numbers than the order of the reaction is considered fractional amd equal to its best estimate;
...
4. if neither the estimates of the slope and the intercept are statistically significant then a different modelling approach must be considered. The package provides a possible non-linear alternative model (FOMT, see other model section)
...
\end{cases}
$$
Once the reation order has been determined, non-linear least squares regression can be performed using the above solution with the rate constant $k$ as free parameter. 
...
The output is an `nls` class object and thus all the functions from package `stats` can be used to retrieve gegression information. Even in this case the goodness-of-fit can be accessed through the  `goodness_of_fit()` function. 
...
plot_ord(ord.cqa)
```
Please remind that the `kin_regr()` function returns the kinetic regression on kinetic data. Information can be retrivered to eventually check if the regression is indeed reliable:
```{r}
